...
export default function Dashboard() {
  ...
  return(
    <main style={{background:'#F8F4EE',minHeight:'100vh',fontFamily:"'DM Sans', sans-serif"}}>
      ...
      <div style={{padding:'24px 28px 48px',maxWidth:'1100px',margin:'0 auto'}}>
        ...
        {activeTab==='home_details'&&(
          <div>
            ...
            {[{key:'about',title:'About This Home'},{key:'features',title:'Structure & Features'}].map(sec=>(
      ...
      {/* MODALS */}
      {showHistoryModal&&(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:500,display:'flex',alignItems:'center',justifyContent:'center',padding:'20px'}}><div style={{background:'#fff',borderRadius:'16px',padding:'36px',width:'100%',maxWidth:'480px',textAlign:'center'}}><div style={{fontSize:'48px',marginBottom:'16px'}}>🏠</div><h2 style={{fontFamily:"'Playfair Display', Georgia, serif",fontSize:'24px',fontWeight:400,color:'#1E3A2F',marginBottom:'10px'}}>This home has a history</h2><p style={{fontSize:'14px',color:'#8A8A82',lineHeight:1.7,marginBottom:'24px'}}>There are {jobs.length} contractor job{jobs.length!==1?'s':''} already logged for this home.</p><div style={{display:'grid',gap:'10px'}}><a href="/log" style={{display:'block',background:'#1E3A2F',color:'#F8F4EE',padding:'12px',borderRadius:'10px',fontSize:'14px',fontWeight:500,textDecoration:'none',textAlign:'center',fontFamily:"'DM Sans', sans-serif"}} onClick={()=>setShowHistoryModal(false)}>See this home&apos;s history first</a><button onClick={()=>setShowHistoryModal(false)} style={{background:'none',border:'1px solid rgba(30,58,47,0.2)',color:'#1E3A2F',padding:'12px',borderRadius:'10px',fontSize:'14px',cursor:'pointer',fontFamily:"'DM Sans', sans-serif"}}>Start my new homeowner guide</button></div></div></div>)}
