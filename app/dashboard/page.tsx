...
export default function Dashboard() {
  ...
  const saveSystem=async(sysId:string)=>{
    ...
    try{
      ...
      if(payload.condition)payload.condition=payload.condition.charAt(0).toUpperCase()+payload.condition.slice(1).toLowerCase()
      ...
      await recalculateScore()
      if(payload.considering_replacing){
        ...
        const alreadyExists=existingProjects.data?.some((p:any)=>p.title.toLowerCase().includes(sysName.toLowerCase()))
        if(!alreadyExists){
                  ...
                  <div style={{padding:'20px'}}>
                    {editingHomeSection==='about'?(
                      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px'}}>
                        ...
                        <div style={{gridColumn:'1/-1'}}><label style={{display:'block',fontSize:'11px',color:'#8A8A82',marginBottom:'6px'}}>HOA</label><div style={{display:'flex',gap:'8px'}}>{[{value:true,label:'🏘️ Yes'},{value:false,label:'No'}].map(opt=>(<button key={String(opt.value)} type="button" onClick={()=>setHomeEdits((p:any)=>({...p,has_hoa:opt.value}))} style={{padding:'7px 16px',borderRadius:'8px',fontSize:'13px',border:`1px solid ${homeEdits.has_hoa===opt.value?'#1E3A2F':'rgba(30,58,47,0.2)'}`,background:homeEdits.has_hoa===opt.value?'#1E3A2F':'#fff',color:homeEdits.has_hoa===opt.value?'#F8F4EE':'#1E3A2F',cursor:'pointer',fontFamily:"'DM Sans', sans-serif",fontWeight:homeEdits.has_hoa===opt.value?500:400}}>{opt.label}</button>))}</div></div>
